...
export default async function handler(
  ...
  const { referralCode, email, password, username } = req.body;
  ...
  // Regex Check
  const regexTestResult = quickRegexCheck(email, password, username);
  ...
  try {
    const userDocSnapshot = await firestore.doc(`showcase/${username}`).get();
...
const quickRegexCheck = (email: string, password: string, username: string) => {
  // Email
  const emailRegex =
